 ...
 * 
 * Automatically deletes security logs older than 1 week.
 ...
 * Run cleanup job
 * 
 * Deletes security logs older than 1 week (7 days)
 * 
...
const runSecurityLogsCleanup = async () => {
  try {
    ...
    if (countError) {
      console.error('❌ Error counting security logs:', countError);
      return {
        success: false,
        deleted: 0,
    ...
    if (logsToDelete === 0) {
      console.log('✅ No security logs to clean up (all logs are within 1 week)');
      return {
        success: true,
        deleted: 0
      };
    ...
    console.log(`📋 Found ${logsToDelete} security log(s) older than 1 week`);
    ...
    if (deleteError) {
      console.error('❌ Error deleting security logs:', deleteError);
      return {
        success: false,
        deleted: 0,
    ...
    console.log(`✅ Security logs cleanup completed: Deleted ${logsToDelete} log(s) (${duration}ms)`);
...
const startSecurityLogsCleanupScheduler = (intervalDays = 7) => {
  console.log(`⏰ Starting security logs cleanup scheduler (every ${intervalDays} days)`);
